...
export class PlatformApprovalFlowExampleComponent implements OnDestroy {
  ...
  toggleSpecificNodeAction(
    field: keyof ApprovalNodeActionsConfig,
    state: boolean
  ): void {
    const approvalProcess = this._approvalFlow.approvalProcess;
    const updatedState = cloneDeep(approvalProcess);
    ...
    approvalProcess.nodes.forEach((node) => {
      if (this.newNodes.has(node.id)) {
        ...
        meta.canAddNodeAfter =
          !this.nodeActionsConfigForNewNodes.disableAddAfter;
        meta.canAddNodeBefore =
          !this.nodeActionsConfigForNewNodes.disableAddBefore;
        meta.canAddParallel =
          !this.nodeActionsConfigForNewNodes.disableAddParallel;
      }
    });
  }
  ...
  setNotStarted(): void {
    ...
    const initialNodeMap = new Map(
      (this.graphs[this.selectedExample] as ApprovalProcess).nodes.map((n) => [
        n.id,
        n,
      ])
    );
...
const teams: () => ApprovalTeam[] = () => [
  {
    ...
    description: '',
    members: ['uid66171', 'uid66161', 'uid66151', 'uid66141'].map((id) =>
      getUser(id)
    ),
  },
  {
    ...
    name: 'Sales team',
    description: '',
    members: ['uid77111', 'uid77115', 'uid77135', 'uid81955'].map((id) =>
      getUser(id)
    ),
  },
  {
...
class UserDataProvider extends DataProvider<ApprovalUser> {
  ...
  ): Observable<ApprovalUser & { phone: string; email: string }> {
